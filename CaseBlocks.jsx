  ...
  return (
    <section style={{
      ...
      <div style={{
        ...
        {/* Display title — set huge, italic. Subtle entry: soft fade in
            with a small upward rise on load. */}
      ...
      {caption && (
        <Reveal kind="rise" delay={200}>
          <div style={{ maxWidth: MAX_W, margin: "0 auto", padding: "18px clamp(20px, 5vw, 56px) 0", display: "flex", flexWrap: "wrap", gap: 8, justifyContent: "space-between", alignItems: "baseline" }}>
            <span style={{ fontFamily: BW.ffD, fontStyle: "italic", fontSize: 14, color: BW.ink3, fontWeight: 400, letterSpacing: "-0.005em" }}>{caption.label || "fig."}</span>
...
function VideoBlock({ block }) {
  const { src, poster, caption, aspect = "16/9", surface = BW.chalk50, surfaceGradient, autoplay = false, muted = false, loop = false, controls = true, maxWidth = 1280, radius = 12, imagePadding = "clamp(32px, 4vw, 56px) clamp(20px, 5vw, 56px)" } = block;
  const bg = surfaceGradient || surface;
    ...
    <section style={{ background: bg, padding: "0 0 60px" }}>
      <Reveal kind="wipe" threshold={0.05}>
        <div style={{ width: "100%", position: "relative", background: bg, padding: imagePadding, boxSizing: "border-box", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <video
            style={{ width: "100%", maxWidth, aspectRatio: aspect, display: "block", background: BW.ink, borderRadius: radius, boxShadow: "0 18px 40px -16px rgba(20,16,12,0.35)" }}
            poster={poster}
            controls={controls}
            autoPlay={autoplay}
